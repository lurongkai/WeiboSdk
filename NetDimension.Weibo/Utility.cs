...
namespace NetDimension.Weibo
{
    ...
    public static class Utility
    {
        ...
        internal static byte[] BuildPostData(string boundary, params WeiboParameter[] parameters) {
            ...
            var headerBuff = Encoding.ASCII.GetBytes(string.Format("\r\n--{0}\r\n", boundary));
            var footerBuff = Encoding.ASCII.GetBytes(string.Format("\r\n--{0}--", boundary));
            ...
            foreach (var p in pairs) {
                if (!p.IsBinaryData) {
                    ...
                    var fileBuff =
                        Encoding.UTF8.GetBytes(string.Format(headerTemplate, p.Name,
                                                             string.Format("upload{0}",
